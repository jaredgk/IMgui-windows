...
def w(s):
    "simple function to make it easier to print to the eps file "
    epsf.write(s + "\n")
...
def rapoint(rpoint):
    """ this if called from a function where the scale has been reset """
...
def curvebox(cdim, cbox, width,color, gray):
...
def arrowa(head,tail,direc, color, gray) :
...
def migrationcurvearrow(val2NM,head,tail,direc, color) :
    ...
    if (direc == 0): ## arrow to the right,  line is shifted up, text is below line
        ...
        arrowheadpoint = [cheadadj[0], head[1] + c2height/2.0]
        if docolor:
    ...
    if (direc == 2): ## arrow to the left, line is shifted down, text is above line
        ...
        ctail = tail
        arrowheadpoint = [cheadadj[0], cheadadj[1] + c2height/2.0]
        if docolor:
            w("%f %f %f setrgbcolor" % (color[0],color[1],color[2]))
...
def calc_scaledvals(slist):
    global numpops
    gentime= slist[6][4][0]
    timeumean = slist[6][4][1]
    scaleumean = slist[6][4][2]
    scaledpop = []
    for i in range(2*numpops-1):
        scaledpop.append(slist[3][4][i][1]/(4.0 * timeumean*gentime))
    scaledtime = []
    for i in range(numpops-1):
        scaledtime.append(slist[4][4][i][1] * (scaleumean/timeumean))
...
def readimfile(imfilename) :
    ...
    global slist
    global numpops
    slist = [["inputfile",False,get_input_file_name,"Text from input file:"],\
...
##             ["migration parameter info",False,get_2NM,"POPULATION MIGRATION (2NM) POSTERIOR PROBABILITY HISTOGRAMS"],\
             ...
             ["demographic scales",False,get_demog_scales,"MARGINAL DISTRIBUTION VALUES IN DEMOGRAPHIC UNITS"],\
##             ["parameter priors",False,get_parameter_priors,"Parameter Priors"] \  ignore this I think
             ]
    slist[6][1] = dontdodemogscale ## skip the demographic scales
    numpops = 0
    ...
    while imfileline != '':
        ...
        for i in range(len(slist)):
            ...
            if slist[i][1] == False and imfileline.upper().find(slist[i][3].upper()) >= 0 :
                thing = slist[i][2](imfile,imfileline,slist[i][3],numpops)
                print 'Input: ',thing,slist[i][3],slist[i][0]
                slist[i].append(thing)
                slist[i][1] = True
...
def yline(y,farright, width, dash, gray) :
    """ draw a line at a specific height in relative terms """
    aline([[0,y],[farright,y]],width, dash, gray)
...
## left side temporarily at zero, right side temporarily at upper confidence interval
        popxvals.append( [0,slist[3][4][i][1]])
    ...
    maxwide = 0
    for i in range(2*numpops-1):
        if maxwide < (popxvals[i][1] + (slist[3][4][i][3]-slist[3][4][i][1])):
            maxwide = (popxvals[i][1] + (slist[3][4][i][3]-slist[3][4][i][1]))
    ...
    for i in range(2*numpops-1):
        confint.append([])
        confint[i].append(minx_popbox + ((popxvals[i][1] - (slist[3][4][i][1]-slist[3][4][i][2]))/maxwide))
        confint[i].append(minx_popbox + ((popxvals[i][1] + (slist[3][4][i][3]-slist[3][4][i][1]))/maxwide))
...
## look at this crazy go around to make a copy of popbox and change it without changing popbox
        ...
        shadowbox[1][0] = popbox[i][1][0] - (slist[3][4][i][1]-slist[3][4][i][2])/maxwide
        w("%%begin left confidence for box %d" % i)
        cdim.append(curvebox(-1,shadowbox,1,color,gray))
        w("%%done left confidence for box %d" % i)
        shadowbox = []
        for j in range(len(popbox[i])):
            shadowbox.append([])
            shadowbox[j].extend(popbox[i][j])
        shadowbox[1][0] = popbox[i][1][0] + (slist[3][4][i][3]-slist[3][4][i][1])/maxwide
        w("%%begin right confidence for box %d" % i)
        cdimtemp =curvebox(cdim[i],shadowbox,1,color, gray)
        w("%%done right confidence for box %d" % i)
    for i in range(2*numpops-1):
        w("%%begin box %d" % i)
        cdimtemp = curvebox(cdim[i],popbox[i],2,color,0)
        w("%%done box %d" % i)
    popprintinc = 0.01
    for i in range(numpops):
        if poptree[i][1] == 1 and i== droppops[1][1] : ## right side of most recent split
            dotext([popbox[i][0][0] + (popbox[i][1][0]-popbox[i][0][0])/2,popbox[i][1][1]+popprintinc],slist[1][4][i],0, False)
        else :
            dotext([popbox[i][0][0],popbox[i][1][1]+popprintinc],slist[1][4][i],0, False)
    popprintinc = 0.025
    if label_a_pops :
...
##    print lastperiod
    ...
    for i in range(2*numpops-1):
        ...
        arrowa(head,tail,2,color,gray)
        head = [confint[i][1],arrowheight]
        tail = [popbox[i][1][0],arrowheight]
        arrowa(head,tail,0, color, gray)
    ...
    if scaledpop != [] :
        ...
        dotext([0.15,0.05]," Ancestral 4Nu: " + str(slist[3][4][rootpop][1]),0, False)
...
def set_tlines(ty,numpops,scaledtime, lastt_lower_y):
    tlowest =  slist[4][4][numpops-2][3] ## bottom of confidence interval of lowest t
    tmax = slist[4][4][numpops-2][3]
    t = []
    for i in range(numpops-1):
        t.append([slist[4][4][i][1],slist[4][4][i][2],slist[4][4][i][3]])
    ty = []
    if localyscale == -1:
...
def print_tlines(ty,numpops,scaledtime, farright):
    ...
    for i in range(numpops-1):
        if (ty[i][1] > ty[i][0]):
            yline(ty[i][1],farright,1,2,gray)
        yline(ty[i][0],farright,1,0,0)
        if (ty[i][2] < ty[i][0]):
            yline(ty[i][2],farright,1,2,gray)
        if(scaledtime != []):
            scaledtime[i] /= 1e6
...
##            str(int(round(scaledtime[i],-int(math.log10(scaledtime[i])-2)))) + " yrs"
            ...
            nstr = fround(slist[4][4][i][1]) + "tu"
            dotext([xinc*(i+2),ty[i][0]+0.001],nstr,0, False)
        if (ty[i][1] > ty[i][0]):
            arrowa([xinc*(i+1),ty[i][1]],[xinc*(i+1),ty[i][0]],1, black, gray)
        if (ty[i][2] < ty[i][0]):
            arrowa([xinc*(i+1),ty[i][2]],[xinc*(i+1),ty[i][0]],3, black, gray)
    return ty
...
def scancommandline():
    ...
    def cflag ():
        ...
        docolor  = True
    def dflag ():
        global dontdodemogscale
        dontdodemogscale  =  True
    def iflag (tempname):
        global imfilename
        imfilename = tempname
    def oflag (tempname):
    ...
    cldic = {'a':aflag,'b':bflag,'c':cflag,'d':dflag,'f':fflag,\
             'g':gflag,'h':hflag,'i':iflag,'m':mflag,'o':oflag,\
             'p':pflag, 's':sflag, 't':tflag,'y':yflag,\
             'x':xflag}
...
def printcommandset():
    ...
    print "-c color, blue for pop size, red for migration (default grayscale)"
    ...
    print "     default value = 1/# sampled populations"
    print "-x adjust width of plot,  >1 means wider, <1 means narrower"
    print "-y adjust height of splittimes, relative to bottom of figure, max = 1."
...
def setbasexyscale():
    ...
    minimumxpoint = minimumypoint = 36.1
    maxll = [minimumxpoint,minimumypoint]
    maxur = [maximumxpoint,maximumypoint]

def setdefaults():
    global numpops, label_a_pops, docolor, dosquare, imfilename, outputfilename
    ...
    global lastt_lower_y, set_lastt_lower_y, slist, blue, red, black, gray
    global popboxspaceadj, moption, dontdodemogscale
    numpops = 0
    label_a_pops = False
    docolor = False
    dosquare = False
...
print "IMfig program.  Copyright 2009-2012  Jody Hey "
setdefaults()

cmdstr = ""
##cmdstr = "IMfig.py -iPan_troglodytes_3pops_3_9_09_5loci_test4.out -odebug.eps"
cmdstr ="IMfig.py -ioreo_chim3M.out -odebug.eps -d"

cmdstr =r"IMfig.py -inoPan_v2_tot4_expo_mprior_1_5_2012.out -onoPan_v2_tot4_expo_mprior_1_5_2012.eps -d -g0.7 -b10"
cmdstr = r"IMfig.py -iNF2ppssr.out -odebug.eps -d"
cmdstr = ""
##//////////// identify if program is run within interpreter, or from command line ///////////
##print "scan commandline"
...
(poptree,rootpop,poptreestring,plist, droppops,addpop) = poptreeread(slist[2][4],numpops)
...
print_mcurves(slist,numpops, popbox, plist, docolor)
epsf.close()
from PIL import Image, _imaging
jpgn = outputfilename.replace('eps','jpg')
i = Image.open(outputfilename)
i.save(jpgn)
