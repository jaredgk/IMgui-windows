...
function validatePaths() {
    ...
    var fl = 0;
    postJson.infile = getName('#infile');
    if($('#c4').is(':checked')) { postJson.paramf = getName('#paramf'); }
    if($('#r1').is(':checked')) { postJson.titag = getName('#titag'); }
    if($('#r4').is(':checked')) { postJson.mcftag = getName('#mcftag'); }
    if($('#c3').is('checked')) { postJson.nestedf = getName('#nestedf'); }
    $.post('/',postJson, function(data) {
        if(data.infile === "1") {
            fl = 1;
            errmsg(postJson.infile + ' is not a valid path for input file');
            $('#in-div').addClass('bg-danger');
        }
        if(data.paramf === "1") {
            fl = 1;
            errmsg(postJson.paramf + ' is not a valid path for the parameter file');
            $('#c4-text').addClass('bg-danger');
            $('#b2').addClass('btn-danger');
        }
        if(data.titag === "1")  {
            fl = 1;
            errmsg(postJson.titag + ' is not a valid path for the .ti file');
            $('#b5').addClass('btn-danger');
            $('#r1_text').addClass('bg-danger');
        }
        if(data.mcftag === "1") {
            fl = 1;
            errmsg(postJson.mcftag + ' is not a valid path for the .mcf file');
            $('#b5').addClass('btn-danger');
            $('#r4_text').addClass('btn-danger');
        }
        if(data.nestedf === "1") {
            fl = 1;
            errmsg(postJson.nestedf + ' is not a valid path for the nested model file');
            $('#b2').addClass('btn-danger');
